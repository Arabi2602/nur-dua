...
export default function DuaWebsite() {
  ...
  const handlePlayAudio = (audioUrl, title) => {
    if (!audioUrl) {
      alert("দুঃখিত, এই দোয়ার অডিও ফাইলটি এখনো যুক্ত করা হয়নি!");
      return;
    }
    ...
    audioRef.current.play().catch(error => {
      ...
      alert("অডিও প্লে করতে সমস্যা হয়েছে। দয়া করে লিংকটি চেক করুন।");
      setPlayingTitle(null);
    });
  ...
  // ক্যাটাগরি ফিল্টার করার সময় ডাটা ডিফেন্সিভ রাখা ভালো
  const categories = ["সব", ...new Set(duasData.map(dua => dua.category).filter(Boolean))];

  // ফিক্সড ফিল্টারিং লজিক (ক্র্যাশ প্রিভেনশনসহ)
  const filteredDuas = duasData.filter((dua) => {
    ...
    // যদি সার্চবক্স খালি থাকে তবে ক্যাটাগরি ম্যাচ করলেই দেখাবে
    if (!searchLower) return matchesCategory;

    // প্রতিটি ফিল্ড চেক করার আগে তা এক্সিস্ট করে কিনা নিশ্চিত করা (ক্র্যাশ থেকে বাঁচতে)
    const titleMatch = dua.title?.toLowerCase().includes(searchLower);
    const meaningMatch = dua.meaning?.toLowerCase().includes(searchLower);
    const pronunciationMatch = dua.pronunciation?.toLowerCase().includes(searchLower);
    
    // tags অ্যারে ডিফেন্সিভ চেক
    const tagsMatch = Array.isArray(dua.tags) && dua.tags.some(tag => 
      typeof tag === 'string' && tag.toLowerCase().includes(searchLower)
    );

    return matchesCategory && (titleMatch || meaningMatch || pronunciationMatch || tagsMatch);
  });
      ...
      <main className="flex-grow">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10 lg:py-24 grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          <div className="lg:col-span-7 text-center lg:text-left">
            ...
            <h2 className="mt-6 text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-[1.15] tracking-tight">
              ...
              <span className={`bg-gradient-to-r ${darkMode ? 'from-emerald-400 to-teal-300' : 'from-emerald-700 to-teal-600'} bg-clip-text text-transparent`}>আকাঙ্ক্ষিত দোয়া</span>।
            </h2>
            ...
            <p className="mt-6 text-sm sm:text-base md:text-lg text-gray-600 dark:text-slate-400 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-medium">
              আরবি, বাংলা উচ্চারণ ও বিশুদ্ধ অর্থসহ প্রয়োজনীয় দোয়ার এক আধুনিক ওয়েব প্ল্যাটফর্ম। নিচের লাইভ স্মার্ট সার্চবক্সে শুধু টাইপ করুন।
            </p>
          ...
          <div className="lg:col-span-5 w-full max-w-md mx-auto">
            <div className={`shadow-2xl rounded-[32px] p-6 sm:p-8 border transition-all ${darkMode ? 'bg-slate-900/60 border-slate-800/80' : 'bg-white border-emerald-100/70'}`}>
              ...
              >
                {playingTitle === 'daily' ? '🛑 স্টপ করুন' : '🔊 অডিও তেলাওয়াত শুনুন'}
              </button>
            </div>
          </div>
        </section>
        ...
        <section id="categories" className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="text-center">
            ...
            <p className="mt-1.5 text-xs sm:text-sm text-gray-500 dark:text-slate-400">ক্লিক করে নির্দিষ্ট বিষয়ের দোয়াগুলো দেখে নিন</p>
          </div>
        ...
        <section id="duas" className="py-12 px-4 sm:px-6 max-w-7xl mx-auto">
          {filteredDuas.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm font-bold text-gray-400 mt-2">কোনো দোয়া খুঁজে পাওয়া যায়নি।</p>
            </div>
          )}
        </section>
      </main>

      {/* 5. PREMIUM RESPONSIVE FOOTER */}
